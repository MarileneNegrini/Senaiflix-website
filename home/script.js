...
function
  ...
  //let variável, valor que pode mudar
  //const constante, valor que não pode ser alterado.
  /* quando o comentário é de muitas linhas , então usa esse formato, barra asterisco; no formato usado acima é o comentário de uma única linha (então ele so começa e nãoprecisa fechar...termina quando termina a linha)
...
function carregarFilmes() {
  ...
  for (const filme of filmes) {
    ...
    let novaUERL = `url('../assets/imgs/${filme.poster}')`

    novoCard.style.backgroundImage = novaUERL
...
function aoPesquisarPorTitulo(titulo) {
  ...
  let seriesContainer = document.querySelector("#serie-card-container")
...
function carregarSeries() {
  // #region series
  let serieContainer = document.querySelector("#serie-card-container");
  ...
  for (const serie of series) {
    ...
    let novaUERL = `url('../assets/imgs/${serie.poster}')`
...
function aoFiltrarPorGenero(genero) {
  ...
  let filmesFiltrados = midia.filter(m => {
    if (m.tipo != "filme") {
      return false; //Não filtra esta mídia
    }
  ...
  let seriesFiltradas = midia.filter(m => {
    if (m.tipo != "serie") {
      return false; //Não filtra esta mídia
    }
    let encontrouOGenero = false;
